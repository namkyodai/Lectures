...
library(reshape)
library(reshape2)
df=read_excel("../data/introduction.xlsx",sheet="bio-time-vn",skip = 0)
df<-melt(df,id="activity")
# Grouped
p<-ggplot(df, aes(fill=activity, y=value, x=variable)) + 
  geom_bar(position="dodge", stat="identity")+
  labs(title = "Số Giờ Trung Bình Hàng Tuần cho chuyện ấy", x = "Thời Kì",y = "Số Giờ",fill="Chuyện ấy")+
  theme(axis.text.x=element_text(angle=0, hjust=0.5))+ 
  geom_text(aes(label=value), position=position_dodge(width=0.9), vjust=-0.25)

q<-ggplot(df, aes(fill=variable, y=value, x=activity)) + 
  geom_bar(position="dodge", stat="identity")+
  labs(title = "Số Giờ Trung Bình Hàng Tuần cho chuyện ấy", x = "Chuyện ấy",y = "Số Giờ",fill="Thời Kì")+
