#Truyền đạt thông tin từ các bảng
#trong môi trường R, có rất nhiều cách để trình bày bảng, có thể xuất ra các định dạng khác nhau và có thể trang điểm cho các bảng một cách rất chuyên nghiệp.


# Thể hiện  table với định dạng {flextable}   
NHPI_table4 %>%
  flextable() %>% 
...
# Tạo ra bảng tổng hợp
tbl_month_BC <-
  thedata_BC_Can %>%
  filter(geo == "British Columbia") %>%
  arrange(ref_date) %>%
  # calculate percent change stats --> tính toán phần trăm thay đổi theo tháng
  get_mom_stats() %>%
  get_yoy_stats() %>%
  # pull year and month
  ...
  # just print rows 1 to 13 --> chỉ in ra từ hạng 1 đến 13
  slice(1:13)

print(tbl_month_BC)

# Trình bày bảng với {kableExtra} formatting  
tbl_month_BC %>%
